"""Module to support 123\\SmartBMS (123electric) generation 3.

Project: aiobmsble, https://pypi.org/p/aiobmsble/
License: Apache-2.0, http://www.apache.org/licenses/

Protocol notes (reverse-engineered from the official 123SmartBMS Android app
v4.1.1, .NET MAUI, assemblies 123SmartBMS.dll / 123Connection.dll):
...
class BMS(BaseBMS):
    """123\\SmartBMS gen3 implementation."""
    ...
    def _notification_handler(
        ...
        """Handle notifications: split '\\r'-terminated ASCII lines."""
        self._buffer += data
    ...
    async def _write_cmd(self, command: str) -> None:
        """Send an ASCII command terminated by '\\r'."""
        async with self._wr_lock:
            await self._client.write_gatt_char(
    ...
    async def _init_connection(
        ...
        if self._secret:
            await self._cmd_expect_ok(f"PW{self._secret}!")  # authenticate (4-digit PIN)
        await self._cmd_expect_ok("E!")  # enable live data streaming (fails if not authorized)
    ...
    async def _async_update(self) -> BMSSample:
        """Collect one full data cycle and return it as a sample."""
        self._cells.clear()
        self._values.pop("battery_level", None)
        ...
        try:
            ...
            self._log.debug(
                "incomplete cycle: %i/%i cells, soc=%s",
                len(self._cells),
                self._cell_total,
                "battery_level" in self._values,
            )
